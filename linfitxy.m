 function [ p, saxipbi, xplot, low_alpha, high_alpha, sp] = ...
...
if nbloop==0
   if plotting
    ...
    % hold on      
     ...
     xplot = min(xdata)+(max(xdata)-min(xdata)).*(0-0.8:1./(nhull-1):1+0.8);
     if not(onlyplot)
...
% some inits
if matlabmode
  options = optimset('MaxFunEvals',100000); % to avoid optimset warnings (?)10000
end
